defmodule Landbuyer2025Web.Live.Dashboard.Strategies do
  ...
  def strategy_block(assigns) do
    ...
    <div class="flex justify-between items-center mb-2 ml-2 mb-[18px]">
      ...
      <div class="flex items-center space-x-2 mt-[4px] mr-[6px]">
        <%= if @strategy do %>
          <%= if @strategy.status == "stopped" do %>
            <.icon_button phx_click="start_strategy" d="M5 3v18l15-9L5 3z" />
          <% else %>
            <.icon_button phx_click="stop_strategy" d="M6 6h12v12H6z" />
          <% end %>
          
          <.icon_button phx_click="edit_strategy" d="M3 17.25V21h3.75l11.06-11.06-3.75-3.75L3 17.25z" />
          <.icon_button phx_click="clean_strategy" d="M3 6h18M9 6v12a2 2 0 002 2h2a2 2 0 002-2V6" />
    ...
    <%= if @strategy == nil do %>
      ...
      <% end %>
    <% else %>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-6 ml-4 mt-[8px]">
  ...
  def add_strategy_form(assigns) do
    ...
    for mod <- strategy_modules do
      IO.puts("→ Inspecting #{inspect(mod)}")

      if Code.ensure_loaded?(mod) do
        IO.puts("✔️ #{mod} is loaded")
      else
        IO.puts("❌ #{mod} is NOT loaded!")
      end
    end
    ...
    ~H"""
    <form phx-submit="create_strategy" class="p-4 rounded text-slate-200 -mt-2">
